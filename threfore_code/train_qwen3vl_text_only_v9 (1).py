...
# ========== 绕过 accelerate device_map 训练检测（必须在 import unsloth 之前） ==========
os.environ["ACCELERATE_BYPASS_DEVICE_MAP"] = "true"
import accelerate
accelerate.Accelerator.verify_device_map = lambda self, model: False
print(">>> accelerate verify_device_map 已 patch 为永远返回 False")
# ============================================================================

# ========== 禁用 transformers Trainer 的 _move_model_to_device ==========
# accelerate 已经通过 hooks 管理设备搬运，Trainer 再调用 .to() 会和 meta tensor 冲突
