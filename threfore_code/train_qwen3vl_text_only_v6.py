...
def _patched_mem_get_info(device=None):
    total = 17179869184
    free = 16106127360  # 15GB，尽量让模型全进 GPU
    return (free, total)
...
# ========== 备用：Monkey-patch accelerate 跳过 device_map 训练检查 ==========
try:
    import accelerate
    _orig_prepare_model = accelerate.Accelerator.prepare_model
    def _patched_prepare_model(self, model, device_placement=None, evaluation_mode=False):
        # 强制清除量化标志和 device_map
        ...
        # 直接返回模型，不走 accelerate 的 device_map 检查
        return model
    accelerate.Accelerator.prepare_model = _patched_prepare_model
    print(">>> accelerate prepare_model 已 patch")
except Exception as e:
    print(f">>> accelerate patch 失败: {e}")
# ============================================================================
...
model, tokenizer = FastVisionModel.from_pretrained(
    ...
    max_memory={0: "15GiB", "cpu": "0GiB"},  # 禁止 CPU offload
)

# 再次清除量化标志（确保）
if hasattr(model, 'is_loaded_in_4bit'):
    model.is_loaded_in_4bit = False
    print(">>> 已清除 is_loaded_in_4bit")
if hasattr(model, 'is_loaded_in_8bit'):
    model.is_loaded_in_8bit = False
    print(">>> 已清除 is_loaded_in_8bit")
