...
# ========== 禁用 Unsloth fix_untrained_tokens（Qwen3-VL meta tensor 兼容） ==========
...
def _patched_mem_get_info(device=None):
    total = 17179869184
    # 返回接近 total 的 free，让 accelerate 把模型尽量加载到 GPU
...
CONFIG = {
    ...
    "max_seq_length": 512,  # 显存紧张，先降到 512
    "lora_r": 16,
    "lora_alpha": 16,
    "learning_rate": 2e-4,
    "batch_size": 1,        # 显存紧张，先降到 1
    "grad_accum": 8,        # 总 batch 保持 8
    "max_steps": 3000,
    "warmup_steps": 5,
}
...
# 强制把模型全部搬到 XPU（解决 accelerate 分片到 CPU 的问题）
print(">>> 正在将模型移至 XPU...")
model = model.to("xpu")
print(">>> 模型已移至 XPU")
