...
# 彻底清除所有模块的 hf_device_map，防止 accelerate 检测到 device_map
def _clear_hf_device_map(m):
    if hasattr(m, 'hf_device_map'):
        delattr(m, 'hf_device_map')
        print(f">>> 已清除 {type(m).__name__}.hf_device_map")
    for child in m.children():
        _clear_hf_device_map(child)

_clear_hf_device_map(model)
