...
os.environ["TRITON_CACHE_DIR"] = r"H:\triton_cache"
...
def _patched_build(name, src, tmpdir, library_dirs, include_dirs, libraries, ccflags=None):
    ...
    # 基础编译选项：
    # /LD = 创建 DLL（.pyd 本质就是 DLL）
    # /Zc:__cplusplus 让 MSVC 正确设置 __cplusplus 宏（SYCL 头文件需要）
    cmd.extend(['/nologo', '/O2', '/std:c++17', '/Zc:__cplusplus', '/MD', '/EHsc', '/LD'])
    ...
    # 自动添加 Python libs 目录（python313.lib 通常在这里）
    python_libs = os.path.join(os.path.dirname(sys.executable), 'libs')
    if os.path.exists(python_libs):
        cmd.append(f'/LIBPATH:{python_libs}')

    # 自动添加 Python Library/lib 目录（如果存在）
    python_lib = os.path.join(os.path.dirname(sys.executable), 'Library', 'lib')
...
# 禁用 Triton autotune，避免在 Intel XPU 上卡死
os.environ["TRITON_DISABLE_AUTOTUNE"] = "1"
# 设置 Triton 编译超时（秒），防止无限等待
os.environ["TRITON_COMPILE_TIMEOUT"] = "300"
# 如需诊断 Triton kernel 执行问题，可取消下面注释（极慢，纯 Python 解释器模式）
...
{}"""
    ...
    print(">>> 开始训练...")
    print(">>> 注意：首次运行 Triton kernel 编译可能需要 5-15 分钟，请耐心等待...")
    print(">>> 如果 20 分钟仍无进度，请按 Ctrl+C 中断并反馈")
    sys.stdout.flush()
