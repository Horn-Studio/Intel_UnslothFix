...
# Triton Intel XPU backend 在 Windows 上生成 GCC 风格命令行传给 MSVC，导致 D8021/D9002 错误。
# 这里直接替换 _build，构造正确的 MSVC 命令行。
...
def _patched_build(name, src, tmpdir, library_dirs, include_dirs, libraries, ccflags=None):
    ...
    cmd = [cc, src, '/nologo', '/O2', '/std:c++17', '/Zc:__cplusplus', '/MD', '/EHsc', '/LD']
...
# ==================== Triton 缓存配置 ====================
os.environ["TRITON_CACHE_DIR"] = r"H:/triton_cache"
os.environ["TRITON_DISABLE_AUTOTUNE"] = "1"
import shutil
shutil.os.makedirs(os.environ["TRITON_CACHE_DIR"], exist_ok=True)
# ================================================================================
...
def init_intel_env():
    ...
    triton_cache = os.environ.get("TRITON_CACHE_DIR", r"H:/triton_cache")
